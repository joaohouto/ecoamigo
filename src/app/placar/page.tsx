          ...
          {top.map((entrada, i) => (
            <motion.li
              ...
              className={`flex items-center gap-3 px-5 py-3 ${
                entrada.destaque ? 'bg-yellow-50' : i % 2 === 0 ? 'bg-white' : 'bg-gray-50/60'
              }`}
            >
              <span
              ...
              >
                {entrada.nome}
                {entrada.destaque && (
                  <span className="ml-1 text-xs text-verde-acento font-normal">← você</span>
                )}
              </span>
...
function FormRegistro({
  ...
  const validarLocalmente = (n: string): string | null => {
    ...
    if (!/^[a-zA-ZÀ-ú0-9 ]+$/.test(t)) return 'Use apenas letras, números e espaços.'
    return null
  }
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="px-6 py-5">
        ...
        <button
          type="button"
          onClick={onPular}
          className="mt-4 text-sm text-gray-400 hover:text-gray-600 transition-colors underline-offset-2 hover:underline"
          style={{ fontFamily: 'var(--font-nunito)' }}
        >
...
export default function PlacarPage() {
  ...
  const localDisplay: EntradaDisplay[] = placarLocal
    .slice(0, TOP_N)
    .map((e: EntradaPlacarLocal) => ({ ...e, destaque: e.nome === nomeRegistrado }))
    ...
    <main className="min-h-screen bg-fundo flex flex-col items-center gap-6 px-4 py-8">
      ...
      <div className="w-full max-w-4xl flex items-center justify-between">
        ...
        <div className="flex flex-col items-end gap-2">
          <IndicadorConexao online={online} />
          <Link
            href="/"
            className="text-sm text-marrom-terra hover:text-verde-primario transition-colors"
            style={{ fontFamily: 'var(--font-nunito)' }}
          >
            ← Menu
          </Link>
      ...
      <div className="w-full max-w-4xl flex flex-col gap-4">
          ...
          {(['local', 'global'] as const).map((t) => (
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-sm font-bold transition-colors ${
                aba === t ? 'bg-marrom-terra text-white' : 'bg-white text-marrom-terra'
              }`}
              style={{ fontFamily: 'var(--font-fredoka)' }}
            >
              {t === 'local'
        ...
        <div className="flex gap-4">
          <div className={`flex-1 min-w-0 flex-col ${aba === 'local' ? 'flex' : 'hidden sm:flex'}`}>
            <TabelaPlacar
              titulo="Local"
              icone="🏠"
          ...
          <div className={`flex-1 min-w-0 flex-col ${aba === 'global' ? 'flex' : 'hidden sm:flex'}`}>
            <TabelaPlacar
              titulo="Global"
              icone="🌐"
